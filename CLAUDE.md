...
**Bugs d'intégration résolus (leçons éditeur, à retenir pour les futures anims).**
1. **Le paramètre Animator DOIT être un `Trigger`, pas un `Bool`.** `isDodging` avait été créé en **Bool** → un Bool ne se remet jamais à `false` tout seul, donc `Any State → Dodge` se re-déclenchait à chaque sortie → **roulade en boucle infinie** (un seul `SetTrigger` côté code, mais ré-entrées sans fin). Diagnostiqué au log (`OnDodged` 1×, mais ENTER/EXIT/ENTER du state). Fix : type **Trigger** (`m_Type: 9`), comme `isJumping`/`isHarvesting`. **Vérifier le type au moindre state one-shot qui boucle.**
2. **Root motion à neutraliser (roulade « in place »), PAS à baker.** Le clip Mixamo de roulade a un vrai root motion (translation **+ rotation**). Il faut le **neutraliser** pour que la roulade soit jouée **sur place** et que le déplacement vienne **uniquement du dash CharacterController** (comme Walk/Run) : **`Apply Root Motion` OFF** sur l'Animator de l'avatar **+ Bake Into Pose OFF** sur les 3 sections (`loopBlend*: 0`, comme les clips qui marchent). ⚠️ Piège vécu : **Bake Into Pose ON** fait *translater le mesh* (baké) pendant que le dash bouge le transform → les deux **divergent** → **téléportation** du perso à la fin du clip. Et **Apply Root Motion ON** (avec Bake OFF) fait *piloter le transform par le clip* → spin 360° + déplacement parasite. Le bon combo = **les deux OFF** → root motion extrait et ignoré → roulade sur place.
3. **Orienter le perso vers la direction d'esquive.** La roulade est une anim **vers l'avant** ; sans orientation, elle part « de côté » si on esquive dans une direction ≠ du regard. Fix code : `transform.rotation = Quaternion.LookRotation(_dodgeDir)` au départ du dash.
4. **Finition fin de roulade.** (a) Le déplacement du dash s'arrêtait **net** → ajout d'une **décélération** (`PlayerMovementConfig._dodgeEaseOutSeconds`, SmoothStep sur les dernières X s) pour éteindre le dash en douceur pendant le relevé. (b) L'anim se coupait au relevé → **monter l'`Exit Time`** de la transition `Dodge → Blend Tree` (~0.9-0.95 au lieu de 0.847) pour jouer le relevé, + durée de transition ~0.1 pour un blend doux. Tuning `DodgeSpeed`/`DodgeDurationSeconds` = ressenti (#88).

**Alternatives écartées.** Anim de roulade pilotée par root motion (incompatible avec le CharacterController, cf. 2026-04-26) ; layer d'override partiel (la roulade est full-body, un état one-shot façon Jump suffit).
