...
2. **Deux nombres séparés et colorés** (choix PO) : part de biome (couleur du biome) + part physique (gris), dans une même bulle via **rich text** (`<color=#…>`). Couleurs : Forêt = vert, Plaines = doré, Montagnes = bleu froid, Côte maritime = cyan. Parts arrondies à 0 omises.
