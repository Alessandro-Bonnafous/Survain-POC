...
6. **Anti-martelage : récupération mesurée depuis la FIN du swing.** `_hitCooldown` n'est plus un délai « depuis le début » (qui, plus court que le clip, ne mordait jamais) mais une **récupération armée à la fin** du swing (`EndSwing`). Cadence max = durée du clip + récupération → marteler le clic ne peut plus enchaîner des coups rapprochés. C'est la future **vitesse d'attaque** (→ WeaponData).
