...
2. **Root motion à baker dans la pose.** Le clip Mixamo de roulade a un vrai root motion (translation **+ rotation**) ; `clipAnimations` vide (jamais configuré) → **spin 360°** + dérive (pastille rouge « loop match »). Fix : **Bake Into Pose** sur les 3 sections (Rotation / Position Y / Position XZ), `Loop Time` off, Apply. Le déplacement réel vient du dash CharacterController.
